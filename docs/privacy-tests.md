...
| `test/privacy/bundle-boundaries.test.ts` (schema validation) | `integrity.date_forensics` — the first field to carry any committer-date-derived data — is validated against `schema/bundle.v1.json`'s `additionalProperties: false` object, over a hostile fixture: it can only ever be the four declared numeric aggregates, never a per-commit value. |
| `test/date-forensics.test.ts` (both cases) | `date_forensics` actually separates the two shapes it exists to separate: a scripted full-history replay (fabricated author dates spanning years, every commit object actually written in one ~20-minute sitting) produces `mismatch_ratio`/`committer_burst_ratio` of exactly `1`, while an ordinary history with a small, real partial rebase stays well below `0.5` on both — proving the signal isn't just noise dressed up as a heuristic. |
