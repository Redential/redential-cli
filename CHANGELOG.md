...
- Standalone binaries (issue #87): `redential-macos-arm64`, `redential-macos-x64`, `redential-linux-x64`, `redential-linux-arm64`, `redential-win-x64.exe` — Node SEA executables with the runtime embedded, no Node.js install required. Built by `scripts/build-sea.mjs` (esbuild bundle + Node's official Single Executable Applications mechanism + `postject` injection), attached to each GitHub Release with `SHA256SUMS` and build-provenance attestation by a new `binaries`/`release-binaries` job pair in `release.yml`. `install.sh`/`install.ps1` (`scripts/`) are the auditable, checksum-verifying one-line installers, documented in `docs/install-binaries.md`. `taxonomy.json`/`signatures/*.json` ship embedded as SEA assets — the loader gains a SEA-aware read path (`src/embedded-assets.ts`) that is byte-identical to the existing filesystem path outside a SEA binary; the privacy suite passes untouched. A new `binary-smoke` job in `ci.yml` builds and smoke-tests the binary on every OS whenever a packaging-relevant file changes, or on `workflow_dispatch`.
- Two new devDependencies, both dev-only (never shipped to npm users, `package.json`'s `files` is unchanged): `esbuild` (exact pin `0.28.2`) — the standard JS bundler, maintained by the esbuild org. Honest note: esbuild DOES ship a postinstall script (its well-known platform-binary selector, which resolves and places the right `@esbuild/<platform>-<arch>` optionalDependency binary — no network fetch when the platform optionalDependency is installed, which the lockfile guarantees here; no code outside the esbuild release itself). This isn't new exposure: `package-lock.json` already carried `esbuild@0.21.5` with `hasInstallScript: true` transitively (`vitest` -> `vite` -> `esbuild`) before this change, and that postinstall already ran on every `npm ci` in this repo. CLAUDE.md's "ZERO postinstall scripts" rule governs this repo's own `package.json`, not every dev-only transitive dependency's install step. `postject` (exact pin `1.0.0-alpha.6`) — the tool Node's own SEA docs point to for injecting a bundle into a copy of the `node` binary, maintained by the Node.js build working group, no install scripts, invoked in `scripts/build-sea.mjs` via its local `node_modules` bin path (never `npx`) so the build always uses the exact lockfile-pinned, integrity-checked copy.
