...
describe("structural signal (H7, docs/schema-change-h7.md)", () => {
  ...
  describe("schema/bundle.v1.json mechanics (docs/schema-change-h7.md's 'Backward compatibility')", () => {
    ...
    it("schema_version is the const 1.4.0", () => {
      expect(schema.properties.schema_version.const).toBe("1.4.0");
    });
    ...
    // still declaring the current schema_version (1.4.0 as of the
    // repo.shallow field; unrelated to evidence/confidence).
    it("a scan of a repo with no structural pattern emits entries with no new fields, under the current schema_version", () => {
      expect(noStructuralBundle.schema_version).toBe("1.4.0");
